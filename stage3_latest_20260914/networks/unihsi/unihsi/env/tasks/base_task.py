...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        # double check!
        self.record_video = cfg.get('record_video', True)
        ...
        self.record_video = self.record_video
        self.video_fps = 30
        self.video_frame_count = 0
        self.video_writer = None
        self.camera_handle = None
        self.frames_dir = cfg.get('frames_dir', '/workspace/output/frames')
        ...
        if self.record_video and not self.headless:
            ...
            self.gym.set_camera_location(self.camera_handle, self.envs[0],
                                        gymapi.Vec3(3, 3, 2), gymapi.Vec3(0, 0, 1))
            os.makedirs(self.frames_dir, exist_ok=True)
        
        self.sim_data_dir = cfg.get('frames_dir', '/workspace/output/frames').replace('/frames', '/sim_data')
        self.sim_frame_count = 0
        self.record_sim_data = self.record_video and self.headless
    ...
    def render(self, sync_frame_time=False):
        ...
        if self.record_video and self.camera_handle is not None:
            if not self.headless:
                ...
                if img is not None and img.size > 0:
                    ...
                    frame_path = os.path.join(self.frames_dir, f'frame_{self.video_frame_count:06d}.png')
                    cv2.imwrite(frame_path, img_bgr)
                
                self.video_frame_count += 1
                self.gym.end_access_image_tensors(self.sim)
        
        if self.record_sim_data and self.headless:
            pass
    def _record_sim_state(self):
        import json
        sim_state = {}
        for i, env in enumerate(self.envs):
            ...
            for a_idx in range(num_actors):
                actor = self.gym.get_actor_handle(env, a_idx)
                actor_name = self.gym.get_actor_name(env, actor)
                body_states = []
                num_bodies = self.gym.get_actor_rigid_body_count(env, actor)
                for b_idx in range(num_bodies):
                    body = self.gym.get_actor_rigid_body_handle(env, actor, b_idx)
                    pose = self.gym.get_rigid_transform(env, body)
                    body_states.append({
                        'p': [pose.p.x, pose.p.y, pose.p.z],
                        'r': [pose.r.x, pose.r.y, pose.r.z, pose.r.w]
                    })
                actor_states[actor_name] = body_states
            sim_state[f'env_{i}'] = actor_states
