...
The fixed authority is hsi.memory.authority.admit_episode, never a caller gate.
Production writes are unavailable until the complete real-motion authority is ported.
...
from hsi.common import artifacts as _artifacts
from hsi.common.artifacts import (SCHEMA_HASH_FIELD, artifact, canonical_bytes, digest,
                           fsync_directory, read_sealed, require, verified, write_once)

STORE_SCHEMA = "p555.memory_store.v2"
SNAPSHOT_SCHEMA = "p555.positive_memory_snapshot.v2"
CYCLE_SCHEMA = "p555.frozen_memory_cycle.v2"
EVENT_SCHEMA = "p555.memory_journal_event.v2"
...
# The numerical/transaction policy is retained. This new static source closure
# deliberately does not claim the old deployed admission-authority identity.
_AUTHORITY_FILES = ("store.py", "authority.py", "schema.py", "extraction.py",
                    "identity.py", "geometry.py", "surface.py")
AUTHORITY_SOURCES = tuple(artifact(Path(__file__).with_name(name)) for name in _AUTHORITY_FILES) + (artifact(_artifacts.__file__),)
POLICY.update(fixed_admission_authority="hsi.memory.authority.admit_episode",
...
def require_authority_sources():
    require(tuple(artifact(row["path"]) for row in AUTHORITY_SOURCES) == AUTHORITY_SOURCES,
            "Consolidated admission/store/extraction sources changed")
...
class MemoryStore:
    ...
    def record_attempt(self, cycle, admission_request_path, proposed_records=()):
        from .authority import admit_episode
