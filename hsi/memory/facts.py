...
from hsi.common.artifacts import artifact, verified, read_sealed, read_json, write_once, digest, require
from .surface import describe_surface, validate_descriptor, load_obj_world_zup, _indices
...
def build_scene_facts(final_scene_path, output_dir):
                ...
                try:
                    ...
                    for candidate in shape["direction_hypotheses"]["candidates"]:
                        ...
                        metadata.update(entry_id=entry_id, scene_id=geometry["scene_id"], instance_id=key,
                            ...
                            source_code=artifact(Path(__file__).with_name("surface.py")))
    ...
    result = {"schema": SCHEMA, "scene_id": geometry["scene_id"], "sources": sources,
              ...
              "source_codes": [artifact(__file__), artifact(Path(__file__).with_name("surface.py"))]}
...
def load_scene_facts(receipt_path):
    ...
    for row in rows.values():
        ...
        for record in row["descriptors"]:
            ...
            from .surface import local_frame
